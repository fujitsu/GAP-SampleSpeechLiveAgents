...
自動的にテナント名とクライアントIDを入力する [ 設定 ] 画面を表示します。
   ...
   - 事前準備で有効化した Azure Speech のリージョン「JapanEast」とキーの値を設定
   - [OK] をクリックして保存してください。
...
#### 逐次送信
- 画面右下の [ 自動送信 ] をチェックして、 OFF にします。
- 画面右下の [ マイク ] アイコンをクリックし、音声を入力します。
  - 【音声認識結果が正しくない】その他の操作をせずに、音声を再度入力します。
  - 【音声認識結果が正しいとき】再度 [ マイク ] アイコンをクリックして、音声認識結果を送信します。
- 送信後、 AI からの回答が返ってくるまでも、音声入力はできますが、 送信は、 AI からの回答がくるまで不可となっています。

#### 自動送信
- 画面右下の [ 自動送信 ] をチェックして、 ON にします。
- 画面右下の [ マイク ] アイコンをクリックし、音声を入力します。
- 発話が確定したら（息継ぎをしたら）、 **自動的に音声認識結果を送信** します。
- 送信後、 AI からの回答が返ってくるまでも、音声入力はできますが、 送信は、 AI からの回答がくるまで不可となっています。
